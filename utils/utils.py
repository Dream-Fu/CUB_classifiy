...
def to_csv(test_loader, model, file_path, title=['id', 'label']):
    result_list = []
    for ii, (data, ids) in tqdm(enumerate(test_loader)):
        input = data.to(device)
        score = model(input)
        probability = torch.nn.functional.softmax(score, dim=1).detach()
        labels = probability.to('cpu').numpy().argmax(axis=1).tolist()
